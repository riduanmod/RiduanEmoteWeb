...
from game_version import * # Game Versions Imported Here
...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.processor_details = "ARM64 FP ASIMD"
...
async def cHTypE(H):
    return 'Squid' if not H else 'CLan' if H == 1 else 'PrivaTe'
    
async def SEndMsG(H , message , Uid , chat_id , key , iv):
    TypE = await cHTypE(H)
    if TypE == 'Squid': return await xSEndMsgsQ(message , chat_id , key , iv)
    elif TypE == 'CLan': return await xSEndMsg(message , 1 , chat_id , chat_id , key , iv)
    elif TypE == 'PrivaTe': return await xSEndMsg(message , 2 , Uid , Uid , key , iv)
...
async def MaiiiinE():
    ...
    try:
        ...
        print(f"[-] Failed to load bot_config.json: {e}")
        return None
    ...
    OnLineiP , OnLineporT = LoGinDaTaUncRypTinG.Online_IP_Port.split(":")[:2]
    ChaTiP , ChaTporT = LoGinDaTaUncRypTinG.AccountIP_Port.split(":")[:2]
